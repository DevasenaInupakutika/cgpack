!$Id: cgca_m2phys.f90 14 2014-12-01 10:14:16Z mexas $
...
integer( kind=idef ), intent( out ) :: lres, ng
...
! new box size
bsz = real( ir*c, kind=rdef ) / real( lres, kind=rdef )
...
integer( kind=iarr ), allocatable, intent( in ) :: &
 space(:,:,:,:)[:,:,:]
integer( kind=idef ), intent( in ) :: lres
...
real( kind=rdef) ::     &
 rlres,                 & ! convert lres to real
 hcsz                     ! half of the cell physical size.
...
! convert lres to real
rlres = real( lres, kind=rdef )

! vectors in CA CS
bcol = real( ((img-1) * szsp(1:3) + 1), kind=rdef) / rlres
bcou = real( img * szsp(1:3), kind=rdef ) / rlres
