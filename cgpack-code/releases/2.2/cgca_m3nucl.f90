!$Id: cgca_m3nucl.f90 14 2014-12-01 10:14:16Z mexas $
...
if (.not. allocated(coarray)) then
 write (*,'(a,i0)') "ERROR: cgca_nr: image ", thisimage
 write (*,'(a)') "ERROR: cgca_nr: coarray is not allocated"
 error stop 
end if
...
if ( any( coarray(lbr(1):ubr(1),lbr(2):ubr(2),lbr(3):ubr(3),       &
 ...
 write (*,'(a,i0)') "ERROR: cgca_nr: image ", thisimage
 write (*,'(a)') "ERROR: cgca_nr: non-liquid elements in coarray"
 error stop
end if
...
if ( number .lt. 1 ) then
 write (*,'(a,i0)') "ERROR: cgca_nr: image ", thisimage
 write (*,'(a)') "ERROR: cgca_nr: number of nuclei must be 1 or more"
 error stop
end if
...
img1: if (image1) then
 ...
 coarsize = size( coarray( lbr(1):ubr(1), lbr(2):ubr(2), lbr(3):ubr(3), &
                           cgca_state_type_grain), kind=ilrg )

 if ( number .gt. critfract * real( nimages ) * real( coarsize ) ) then
   write (*,'(a)') "WARNING: cgca_nr: too many nuclei - no physical sense!"
   write (*,'(a,g0)') "WARNING: cgca_nr: model size: ", &
                      critfract * real( nimages ) * real( coarsize ) 
   write (*,'(a,i0)') "WARNING: cgca_nr: nuclei: ", number
 end if
 ...
 do
  ...
  ncln: if (coarray(local(1),local(2),local(3),cgca_state_type_grain)  &
   ...
   if (debug) then
    ...
    write (*,'(a,i0)') "DEBUG from cgca_nr: nucleus ",nuc
   end if
