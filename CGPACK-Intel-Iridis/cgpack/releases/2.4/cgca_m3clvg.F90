!$Id: cgca_m3clvg.F90 71 2015-03-25 15:33:10Z mexas $
...
!    cgca_clvgp1, cgca_dacf, cgca_gbupd
!  USES
...
! counter of a gcupd pair, always start from 1
  ...
  clv: if ( any( cgca_clvg_states .eq. farr(x1,x2,x3)) ) then
    ...
    same: if ( marr(centre,centre,centre) .eq. marr(x1,x2,x3) ) then
      ...
      crossgb: if ( intact .and.                                       &
        ...
        ! Add the (grain, neighbour) pair to gcupd coarray on this image
        gcupd( gcucnt, : ) = (/ marr(centre,centre,centre),            &
                marr(x1,x2,x3), cgca_gb_state_fractured /)
...
!    cgca_clvgp
!  NOTES
!    not for ifort due to the use of CO_SUM
#ifndef IFORT
!  SYNOPSIS
...
!*robodoc*s* cgca_m3clvg/cgca_gcupd
!  NAME
!    cgca_gcupd
!  SYNOPSIS  

subroutine cgca_gcupd

!  SIDE EFFECTS
!    State of GB array in module cgca_gb is updated
!  DESCRIPTION
...
integer( kind=idef ) :: i, j, img, nimgs
integer( kind=kind(gcupd) ) :: gcupd_local(3)
