!$Id: cgca_m2alloc.f90 82 2015-03-30 09:09:37Z mexas $
...
!  DESCRIPTION
! Module dealing with the allocation and deallocation of various arrays.
! Several routines are used because they allocate arrays with different
! dimensionality, i.e.
!        (:)[:,:,:]
!    (:,:,:)[:,:,:]
!  (:,:,:,:)[:,:,:]
...
!  SIDE EFFECTS
!    coarray becomes allocated with all values assigned cgca_liquid_state.
!  DESCRIPTION
...
! number or the cell state. The routine increases extent by 2 in each
! direction, thus creating space for storing halos. No check for
! the validity of the coarray dimension is made here. The user should
! make sure the coarray dimension values passed to the routine make
! sense.
!  NOTES
...
if ( .not. allocated(coarray) ) allocate( coarray(            &
 l1-halo:u1+halo, l2-halo:u2+halo, l3-halo:u3+halo, props)    &
  [col1:cou1, col2:cou2, col3:*], source=cgca_liquid_state, stat=errstat)
if (errstat .ne. 0) then
 write (*,'(a,i0)') "ERROR in cgca_as: image: ", this_image()
 write (*,'(a)')    "ERROR in cgca_as: cannot allocate coarray"
 write (*,'(a,i0)') "ERROR in cgca_as: error code: ", errstat
 error stop
end if
...
if (allocated(coarray)) then
 ...
 if (errstat .ne. 0) then
  write (*,'(a,i0)') "ERROR in cgca_ds: image: ", this_image()
  write (*,'(a)')    "ERROR in cgca_ds: cannot deallocate coarray"
  write (*,'(a,i0)') "ERROR in cgca_ds: error code: ", errstat
  error stop
 end if
else
...
if (errstat .ne. 0) then
 write (*,'(a,i0)') "ERROR in cgca_av: image: ", this_image()
 write (*,'(a)') "ERROR in cgca_av: cannot allocate coarray"
 write (*,'(a,i0)') "ERROR in cgca_av: error code: ", errstat
 error stop
end if
...
if (allocated(coarray)) then
 deallocate(coarray,stat=errstat)
 if (errstat .ne. 0) then
  write (*,'(a,i0)') "ERROR in cgca_dv: image: ", this_image()
  write (*,'(a)') "ERROR in cgca_dv: cannot deallocate coarray"
  write (*,'(a,i0)') "ERROR in cgca_dv: error code: ", errstat
  error stop
 end if
end if
...
if (errstat .ne. 0) then
  write (*,'(a,i0)') "ERROR in cgca_art: image: ", this_image()
  write (*,'(a)') "ERROR in cgca_art: cannot allocate coarray"
  error stop
end if
...
if (allocated(coarray)) then
  deallocate(coarray,stat=errstat)
  if (errstat .ne. 0) then
    write (*,'(a,i0)') "ERROR in cgca_drt: image: ", this_image()
    write (*,'(a)') "ERROR in cgca_drt: cannot deallocate coarray"
    error stop
  end if
end if
