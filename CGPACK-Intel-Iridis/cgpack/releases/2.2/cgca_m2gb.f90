!$Id: cgca_m2gb.f90 14 2014-12-01 10:14:16Z mexas $
...
integer :: errstat=0, x1, x2, x3, n1, n2, n3, lbr(4), ubr(4), pair, con
...
if (.not. allocated(coarray)) then
  write (*,'(a,i0)') "ERROR: cgca_gc: image: ", this_image()
  write (*,'(a)') "ERROR: cgca_gc: coarray is not allocated"
  error stop
end if
...
! if not already allocated, allocate to zero length!
...
! this should be replaced by MOVE_ALLOC!!!
    ...
    if (errstat .ne. 0) then
      write (*,'(a,i0)') "ERROR: cgca_gc: image: ", this_image()
      write (*,'(a,i0)') &
        "ERROR: cgca_gc: cannot allocate tmp, error code: ", errstat
      error stop
    end if
...
!********************************************************
  ...
  if (errstat .ne. 0) then
    write (*,'(a,i0)') "ERROR: cgca_gcu: image: ", this_image()
    write (*,'(a)') "ERROR: cgca_gcu: cannot deallocate tmp"
    write (*,'(a,i0)') "ERROR: cgca_gcu: error code: ", errstat
    error stop
  end if
...
if ( errstat .ne. 0 ) then
  write (*,'(a,i0)') "ERROR: cgca_gcp: image: ", img
  write (*,'(a,i0)') &
    "ERROR: cgca_gcp: cannot open file, error code: ", errstat
  error stop
end if
...
do i = 1, ubound( gc, dim=1 )
  ...
  if ( gc(i,3) .eq. cgca_gb_state_intact) then
    ...
    write (*,'(a,i0)') "ERROR: cgca_gcp: image: ", img
    write (*,'(a,i0)') "ERROR: cgca_gcp: state is neither&
      & intact nor fractured: ", gc(i,3)
    error stop
  end if
  ...
  if ( errstat .ne. 0 ) then
    write (*,'(a,i0)') "ERROR: cgca_gcp: image: ", img
    write (*,'(a,i0)') &
      "ERROR: cgca_gcp: cannot write to file, error code: ", errstat
    error stop
  end if
...
allocate( gc(len,3), source=0_idef, stat=errstat )
if ( errstat .ne. 0 ) then
  write (*,'(a,i0)') "ERROR: cgca_agc: image: ", this_image()
  write (*,'(a,i0)') &
    "ERROR: cgca_agc: cannot allocate gc, error code: ", errstat
  error stop
end if
...
if (errstat .ne. 0) then
  write (*,'(a,i0)') "ERROR: cgca_dgc: image: ", this_image()
  write (*,'(a,i0)') &
    "ERROR in cgca_dgc: cannot deallocate gc, error code: ", errstat
  error stop
end if
...
if ( g1 .eq. g2 ) then
  write (*,'(2(a,i0),tr1,i0,".")') "WARN: cgca_gcf: image ", img, &
    ": identical grain numbers for the pair: ", g1, g2
  return
  ! error stop
end if
...
do i=1,ubound(gc,1)
 mtch: if ( gc(i,1) .eq. grain1 .and. gc(i,2) .eq. grain2 ) then
  ...
  if ( gc(i,3) .eq. cgca_gb_state_intact) then
   ...
   write (*,'(a,i0)') "ERROR in cgca_gcr: image ", img
   write (*,'(a)')    &
    "ERROR in cgca_gcr: the state is neither intact nor fractured."
   write (*,'(a)')    &
    "ERROR in cgca_gcr: The integrity of the gc array is broken."
   error stop
  end if
 end if mtch
end do
